package android.phuquy.dev.cashmedia.Utils;

import android.phuquy.dev.cashmedia.R;

public class Config {
	public static final String YOUR_EMAIL = "cashapp@gmail.com";
	public static final String LINK_TO_SHARE = "http://google.com.vn";
	
	//do not change this
	public static final String TAG_CONTROLLER = "controller";
	public static final String TAG_SUCCESS = "success";
	public static final String TAG_MESSAGE = "message";

	public static final int NET_ICON[] = {R.drawable.ic_supersonic,R.drawable.ic_matomy,R.drawable.ic_radium,R.drawable.ic_launcher};
	public static final String NET_NAME[] ={"SuperSonic","Matomy","RadiumOne","Super Reward"};
	public static final String[] POINTS = { "1000","2000", "5000", "10000", "25000",
			"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"
			,"1000","2000", "5000", "10000", "25000"};
	public static final int REWARD_ICON [] = {
			R.drawable.ic_paypal,R.drawable.ic_paypal,R.drawable.ic_paypal,R.drawable.ic_paypal,R.drawable.ic_paypal,
			R.drawable.ic_itunes,R.drawable.ic_itunes,R.drawable.ic_itunes,R.drawable.ic_itunes,R.drawable.ic_itunes,
			R.drawable.ic_amazon_version,R.drawable.ic_amazon_version,R.drawable.ic_amazon_version,R.drawable.ic_amazon_version,R.drawable.ic_amazon_version,
			R.drawable.ic_ebay,R.drawable.ic_ebay,R.drawable.ic_ebay,R.drawable.ic_ebay ,
			R.drawable.ic_googleplay,R.drawable.ic_googleplay,R.drawable.ic_googleplay,
			R.drawable.ic_gamestop,R.drawable.ic_gamestop,
			R.drawable.ic_rizty,
			R.drawable.ic_bestbuy,
			R.drawable.ic_trion,
			R.drawable.ic_xbox,
			R.drawable.ic_applebee,
			R.drawable.ic_gap,
			R.drawable.ic_sears,
			R.drawable.ic_starbucks,
	};
	public static final String[] PP_REWARDS = {
			"1$ Paypal USD Cash","$2 Paypal USD Cash","$5 Paypal USD Cash", "$10 Paypal USD Cash", "$25 Paypal USD Cash",
			"1$ iTunes USD Cash","$2 iTunes Canada Gift USD Cash","$5 iTunes USD Cash", "$10 iTunes Canada Gift USD Cash", "$25 iTunes USD Cash",
			"$1 Amazon.com Gift Cash","$2 Amazon.com Gift Cash","$5 Amazon.com Gift Cash", "$10 Amazon.com Gift Cash", "$25 Amazon.com Gift Cash",
			"$2 eBays Gift Cash","$5 eBays Gift Cash", "$10 eBays Gift Cash", "$25 eBays Gift Cash",
			"$5 Google Play Gift Cash", "$10 Google Play Gift Cash", "$25 Google Play Gift Cash",
			"$5 GameStop Gift Cash", "$10 GameStop Gift Cash",
			"$5 Rixty Gift Cash",
			"$5 Trion Gift Cash",
			"$5 Best Buy Gift Cash",
			"$5 XBox Digital Gift Cash",
			"$5 Gap Gift Cash",
			"$5 Applebee's Gift Cash",
			"$10 Sears Gift Cash",
			"$10 Starbucks Gift Cash",

	};
	
	public static final String PREFERENCES_NAME = "cashapp";
	public static final String PREFERENCES_KEY = "whatthefuck";
	public static final String KEY_LOGGED = "logged";
}
